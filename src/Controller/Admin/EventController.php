...
class EventController extends AbstractController
{
    ...
    private function validateEventDate(string $date): ?string
    {
        ...
        if (!\DateTime::createFromFormat('Y-m-d', $date)) return 'Format de date invalide.';
        return null;
    }
    ...
    public function createSave(Request $request, EntityManagerInterface $em): Response
    {
        ...
        $conn->executeStatement(
            "INSERT INTO events (title, location, event_date, max_participants, link, created_at, creator_id)
             VALUES (:title, :location, :eventDate, :maxParticipants, :link, NOW(), 1)",
