...
public class GererRendezVousController implements Initializable {
    ...
    // ── État ─────────────────────────────────────────
    private int psychologueId = 6; // sera surchargé via setPsychologueId()
    ...
    private void loadRdvDuJour(LocalDate date) {
        ...
        try {
            ...
            pst.setInt(1, psychologueId);
            pst.setDate(2, java.sql.Date.valueOf(date)); // on passe le jour sélectionné
            ResultSet rs = pst.executeQuery();
            ...
            while (rs.next()) {
                ...
                Rectangle bar = new Rectangle(4, 36,
                        getStatutColor(rs.getString("statut")));
                bar.setArcWidth(4); bar.setArcHeight(4);

                VBox info = new VBox(2);
                Label nameLabel = new Label("👤 " + rs.getString("firstname") + " " + rs.getString("lastname"));
                nameLabel.setStyle("-fx-font-weight: bold; -fx-font-size: 12px; -fx-text-fill: #1a3a1a;");
        ...
        new Thread(() -> {
            try {
                if (!GoogleCalendarService.isAuthenticated()) {
                    Platform.runLater(() -> {
                        googleAuthBox.setVisible(true);
                        googleAuthBox.setManaged(true);
                ...
                // Récupérer tous les RDV sans google_event_id (non encore synchronisés)
                Connection cnx = MyBDConnexion.getInstance().getCnx();
                PreparedStatement pst = cnx.prepareStatement(
                        "SELECT id, dateRendezVous, heureDebut, heureFin, type_seance " +
                                "FROM rendez_vous WHERE medecinId = ? AND google_event_id IS NULL");
                pst.setInt(1, psychologueId);
                ResultSet rs = pst.executeQuery();
                ...
                while (rs.next()) {
                    ...
                    try {
                        String titre    = "🩺 Séance " + typeSeance + " — Nafseyti";
                        String googleId = GoogleCalendarService.createEvent(titre, dateRdv, debut, fin, typeSeance);
                        ...
                        System.err.println("⚠️ Erreur sync RDV id=" + rdvId + " : " + ex.getMessage());
                    }
                }

                int totalSynced = synced;
                Platform.runLater(() -> {
                    syncStatusLabel.setText("✅ " + totalSynced + " RDV synchronisé(s) avec Google");
                    syncDot.setFill(Color.web("#285921")); // point vert (tu peux mettre bleu ici)
                    // Rafraîchir le calendrier pour afficher les points bleus
                    loadRdvFromDB();
                    renderCalendar();
    ...
    private void handleValiderCode() {
        ...
        new Thread(() -> {
            try {
                ...
                Platform.runLater(() -> {
                    authErrorLabel.setText("❌ Code invalide. Réessayez.");
                    authErrorLabel.setStyle("-fx-text-fill: #c0392b; -fx-font-size: 11px;");
                    syncStatusLabel.setText("❌ Échec authentification");
                    syncDot.setFill(Color.web("#E53935"));
                });
            }
        }).start();
    ...
    private void handleBtnAccueil() {
        navigateTo("/fxmlPsychologue/PsychologueInterface.fxml");
    }
    ...
    private void handleBtnFiches() {
        try {
            ...
            ctrl.setPsychologueId(psychologueId);
